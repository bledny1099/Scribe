...
struct VocabularySettingsView: View {
    ...
    @State private var selectedPresetCategory: String = "vocabulary"
    ...
    private func deletePreset(_ preset: VocabularyPreset, removeWordsFromActive: Bool) {
        ...
        } else if preset.category == "blocked" {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    VStack(spacing: 10) {
                        HStack(alignment: .center, spacing: 12) {
                            ...
                            HStack(spacing: 8) {
                                Button(action: {
                                    presetModalCategory = selectedPresetCategory
                        ...
                        HStack(spacing: 6) {
                            let categories: [(id: String, title: String, icon: String, count: Int)] = [
                                ("vocabulary", appState.l("Vocabulary"), "text.quote", appState.customVocabularyPresets.count),
                                ("blocked", appState.l("Blocked Words"), "nosign", appState.customBlockedWordsPresets.count),
                                ("location", appState.l("Locations"), "mappin.and.ellipse", appState.customLocationPresets.count)
                            ]

                            ForEach(categories, id: \.id) { cat in
                                let isSelected = selectedPresetCategory == cat.id
                                Button(action: {
                                    ...
                                    HStack(spacing: 5) {
                                        ...
                                        Text("\(cat.count)")
                                            ...
                                            .padding(.horizontal, 5)
                                            .padding(.vertical, 1)
                                            .background(isSelected ? Color.primary.opacity(0.15) : Color.primary.opacity(0.06))
                                            .cornerRadius(6)
                                    }
                                    .padding(.horizontal, 10)
                                    .padding(.vertical, 5)
                    ...
                    // Content for selected preset category
                    if selectedPresetCategory == "vocabulary" {
                    ...
                    } else if selectedPresetCategory == "blocked" {
                    ...
                    } else if selectedPresetCategory == "location" {
...
public final class BugReportService: ObservableObject, @unchecked Sendable {
    ...
    public func sendReport(description: String, includeLogs: Bool = true, userNickname: String = "") async -> Bool {
        ...
        let appVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "2.6.5"
        let buildNumber = Bundle.main.infoDictionary?["CFBundleVersion"] as? String ?? "2.6.5"
        let osVersion = ProcessInfo.processInfo.operatingSystemVersionString
...
struct SupporterCelebrationOverlayView: View {
    ...
    var body: some View {
        ZStack {
            // Layer 0: Calm darkened backdrop with native blur
            Color.black
                .opacity(0.60)
                .background(.ultraThinMaterial)

            // Layer 1: Subtle, calm ambient backlight centered behind crest
            RadialGradient(
                colors: [primaryColor.opacity(0.12), primaryColor.opacity(0.02), .clear],
                center: .center,
                startRadius: 10,
                endRadius: 200
            )
            ...
            VStack(spacing: 18) {
                ...
                ZStack {
                    Circle()
                        ...
                        .shadow(color: Color.black.opacity(0.25), radius: 14, x: 0, y: 5)

                    Image(systemName: data.tier.icon)
                        .font(.system(size: 30, weight: .semibold))
                        .foregroundStyle(
                            LinearGradient(
                                colors: [Color.white.opacity(0.96), primaryColor],
                                startPoint: .topLeading,
                                endPoint: .bottomTrailing
                            )
                        )
                        .shadow(color: primaryColor.opacity(0.25), radius: 6)
                }
                ...
                .overlay(
                    Capsule()
                        .strokeBorder(primaryColor.opacity(0.25), lineWidth: 0.8)
                )
