...
# A. Close `if selectedTab == .system`
system_end_regex = re.compile(r'(\.sheet\(isPresented: \$showingSupportModal\) \{\s*SupportDeveloperModal\(\)\s*\.environmentObject\(appState\)\s*\})\s*\}', re.DOTALL)
content = system_end_regex.sub(r'\1\n                    }\n                }', content)

# B. Close `HStack`
content = content.replace("            // Header — draggable to move the window", "            } // End of HStack\n            \n            // Header — draggable to move the window")
