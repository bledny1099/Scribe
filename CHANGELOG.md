...
- Direct Typing insertion mode (`PasteMode.directType`) providing zero-clipboard text entry via simulated keystrokes.
- Transient clipboard isolation flags (`org.nspasteboard.TransientType`, `AutoGeneratedType`, `ConcealedType`) preventing third-party clipboard managers from polluting history.
- Studio model tier (`openai_whisper-large-v3`) in model preferences alongside Ultra (Large V3 Turbo), Standard, and Eco.

## Changed
- Enabled continuous 30 FPS waveform visualizer with fast-attack / smooth-decay envelope tracking across 52 spectral bars.
- Lowered audio visualizer noise floor to -78 dB with 0.35 perceptual power curve for lively, responsive feedback on quiet speech.
- Upgraded prompt conditioning to pass full prompt prefixes (`tokens.prefix(180)`), preventing truncation of grammar priming and tech code-switching vocabulary.
